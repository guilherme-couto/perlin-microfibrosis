...
% Specify if boundary elements should create artificial skin (fully
% occupied) or just use no skin in those areas
pad_boundaries = 0;

% Specify if volume fractions should count only accessible "pores", found
% by considering only conductive material that touches any boundary. Given
% the vertex-centred FVM used here allows for "diagonal" communication
% (even when cardinal directions blocked), this is the neighbourhood
% checked when calculating which sites are accessible
Vfrac_accessible = 0;
...
if use_skin
    ...
    % If using padded boundaries, create this padding using no-flux
    % occupied regions around the boundary of the original problem, so skin
    % can be applied even to boundary elements. The skin map will also be
    % consistent so can be defined here
    if pad_boundaries
        occ_map_padded = [true(Sy,Nx + 2*Sx); [true(Ny, Sx), occ_map, true(Ny, Sx)]; true(Sy,Nx + 2*Sx)];
        D_xx_padded = [zeros(Sy,Nx + 2*Sx); [zeros(Ny, Sx), problem.D_tensor.D_xx, zeros(Ny, Sx)]; zeros(Sy,Nx + 2*Sx)];
        D_xy_padded = [zeros(Sy,Nx + 2*Sx); [zeros(Ny, Sx), problem.D_tensor.D_xy, zeros(Ny, Sx)]; zeros(Sy,Nx + 2*Sx)];
        D_yy_padded = [zeros(Sy,Nx + 2*Sx); [zeros(Ny, Sx), problem.D_tensor.D_yy, zeros(Ny, Sx)]; zeros(Sy,Nx + 2*Sx)];
        Vfrac_padded = [zeros(Sy,Nx + 2*Sx); [zeros(Ny, Sx), problem.Vfrac, zeros(Ny, Sx)]; zeros(Sy,Nx + 2*Sx)];
    end
...
for i = 1:NAx
    for j = 1:NAy
        ...
        if Vfrac_accessible
            AVfrac(j,i) = boundaryAccessible(occ_here);
        else
        ...
        if AVfrac(j,i) == 0    % If all sites are occupied in this averaging volume, set diffusivity to zero
            ...
            if use_skin
                ...
                % Processing is coded separately depending on whether padding
                % is used
                if pad_boundaries
                    
                    % With padded boundaries, no checks of location are
                    % required, as lookups can just use the padded data with
                    % extra elements included on all edges. So, the skin
                    % map can be used unchanged, and skin is appended on
                    % all sides for iV and jV
                    iV = [iV, iV(end)+(1:2*Sx)];
                    jV = [jV, jV(end)+(1:2*Sy)];
                    skin_map_here = skin_map;
            ...
            % Set up the sub problem. If padded boundaries were used, we must
            % read from the padded problem
            if pad_boundaries && use_skin
                subproblem.occ_map = occ_map_padded(jV,iV);
                subproblem.D_tensor.D_xx = D_xx_padded(jV,iV);
                subproblem.D_tensor.D_xy = D_xy_padded(jV,iV);
                subproblem.D_tensor.D_yy = D_yy_padded(jV,iV);
                subproblem.Vfrac = Vfrac_padded(jV,iV);
