...
% Initialise a figure to plot a maximum of 16 patterns (4x4 grid)
...
for m = 1:N_patterns
    ...
    % Plot this pattern only if the actual count is less than the maximum
    if m > num_plots_x * num_plots_y
        continue;
    end
    subplot(num_plots_y, num_plots_x, m);
    imagesc(presence);
    axis('equal', 'off');
    title(['s: ', num2str(seed), ' - d: ', num2str(actual_density)], 'FontSize', 18);
    colormap(fibroclr);
end
