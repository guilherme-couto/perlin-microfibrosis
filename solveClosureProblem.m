...
switch boundary_type
    ...
    case 'linear'
        ...
        % Read out which sites are on boundaries
        bound_sites = [AEsites, AWsites, ANsites, ASsites];
        % Zero out these rows
        A(bound_sites,:) = 0;
        b(bound_sites) = 0;
        % Grab out diagonal elements
        Adiag = spdiags(A,0);
        % Overwrite boundary elemnts with a value of 1
        Adiag(bound_sites) = 1;
        % Place this modified diagonal back in the matrix as the diagonal
        A = spdiags(Adiag,0,A);        
